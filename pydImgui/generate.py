...
def parse_enum(cursor):
    out('py::enum_<{cname}>(pyd_imgui, "{pyname}", py::arithmetic())'.format(
        cname=name(cursor),
...
def parse_class(cursor):
    if is_class_mappable(cursor):
        ...
        out('py::class_<{}> {}(pyd_imgui, "{}");'.format(name(cursor), clsname, clsname))
...
if __name__ == '__main__':
    ...
    cindex.Config.set_library_path('C:/Program Files (x86)/LLVM/bin')
    ...
    out.file = open(os.path.join(base, 'out.cpp'), 'w')
    out.indent = 0
    out(HEADER)
    out.indent = 1
